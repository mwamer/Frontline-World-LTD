...
│   └── programmes/
│       └── <programme>.html       # Programmes (served under /training-academy/)
...
| `content/programmes/<slug>.html` with a `url` field | the `url` value     |
...
| `url`     | Overrides the output path. Programmes use it to sit under `/training-academy/programmes/`.     |
...
| `/training-academy/programmes/<slug>/` | `../../../` | `../../training-academy/` | `../../../styles.css` |
...
- To link to a section on the same page, use just the fragment: `#programmes`.
...
1. Copy an existing file in `content/programmes/`.
2. Set `url` in frontmatter to `/training-academy/programmes/<slug>/`.
3. Edit the copy, including the title, audience, and format facts.
4. Add the programme to the list in `content/training-academy.html`.
...
- The programme list on `content/training-academy.html`.

When you add or remove an article or programme, update the matching list by hand.
